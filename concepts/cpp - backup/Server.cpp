...
Server::Server()
{
	...
	std::cout << "listening socket: " << _listening_socket << std::endl;
}
...
void Server::runServer()
{
	...
	while (1)
	{
		...
		for (i = 0; i < nr_events; i++)
		{
			...
			{
				printf("WRITE\n");
				// std::cout << "ident: " << evList[i].ident << std::endl;
				// std::cout << "flags: " << (evList[i].flags & EV_EOF) << std::endl;
				if (evList[i].flags & EV_EOF)
				{
...
void Server::readRequest(struct kevent event)
{
	...
	if (ret < 0)
	{
		std::cout  << " fd read if : " << event.ident << std::endl;
...
void Server::sendResponse(struct kevent event)
{
	...
	// close(event.ident);
	struct kevent evSet;
	EV_SET(&evSet, event.ident, EVFILT_WRITE, EV_DELETE, 0, 0, NULL); 
	if (kevent(_kq, &evSet, 1, NULL, 0, NULL) == -1)
...
int Server::closeClient(struct kevent event, int filter)
{
	...
	std::cout << "filter: " << filter << " fd: " << event.ident << std::endl;
